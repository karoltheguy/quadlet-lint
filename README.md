...
- **`QL080` (port-format values)** is checked only for the port-mapping keys in a hand-curated, source-cited table (`src/ports.ts`) — `PublishPort=` in `[Container]`/`[Pod]`/`[Kube]` and `ExposeHostPort=` in `[Container]`. It fires *only* on a numeric token that provably exceeds the `65535` port ceiling, the one port malformation no newer Podman could ever accept; it deliberately never parses the full `ip:hostPort:containerPort` grammar, so it cannot misfire on IPv6 addresses, port ranges, or `/tcp`-style protocol suffixes. Exactly as with `QL040`, values containing interpolation (`$`, `%`, backticks, `{{`) or spanning continuation lines are never judged, and omission is the safe default.
- **`QL081` (AddHost format)** is checked only for `AddHost=` in `[Container]`/`[Pod]` (`src/addhost.ts`), and fires *only* when the value contains neither `:` nor `=`. `AddHost=` is documented as `hostname:ip`, so a value with no separator at all cannot be a valid mapping; every valid target keeps a `:` (the plain `host:ip`, the `host:host-gateway` special, and unbracketed IPv6 targets), and `=` is exempted because Docker 25 added `hostname=ip` as an accepted separator that a newer Podman may follow. Exactly as with `QL040`/`QL080`, interpolated and continuation values are never judged, and omission is the safe default.
- **`QL082` (byte-size format)** is checked only for `Memory=`/`ShmSize=` (`src/bytesize.ts`), and fires only when the value isn't a number followed by an optional `go-units` suffix (one prefix from `k/m/g/t/p`, optional `i`, optional `b`, case-insensitive, e.g. `512m`, `1.5g`, `512MiB`). This is a closed-set bet on the go-units unit vocabulary, which has been frozen for roughly a decade, so the residual forward-compat risk is negligible (a new SI prefix like exa is both improbable and physically irrelevant for container memory). As with `QL040`/`QL080`/`QL081`, interpolated and continuation values are never judged.
